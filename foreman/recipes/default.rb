...
node[:deploy].each do |application, deploy|
  node[:opsworks][:instance][:layers].each do |layer_name|
    ...
    execute "export upstart config" do
      ...
      Chef::Log.info command "bundle exec foreman export upstart /etc/init --app #{layer_name} --user deploy --log #{deploy[:deploy_to]}/shared/log --procfile deploy/Procfile.#{layer_name} --root #{deploy[:current_path]}"
